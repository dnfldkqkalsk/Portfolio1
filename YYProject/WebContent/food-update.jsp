...
<body>
	<div id="wrap">
		<div id="container">
			...
			<!-- food-write -->
			<!-- foodWrite는 서블릿 매핑값 -->
			<form action="/YYProject/UpdateServlet" method="post"
				name="foodWrite" enctype="multipart/form-data"
				onkeydown="return captureReturnKey(event)">

				<!-- input type="file"는 actionName을 사용하지 못하므로 bNum으로 name값을 지정함 -->
				...
				<div class="food-write">
					...
					<!--  input으로 bNum value값 세팅, input value를 list 가져온 값으로 세팅 -->
					<!-- foodInsert는 팩토리 구분값-->
					...
					<div class="write-right">
						<div class="bName">
							<input type="text" name="bName" required="required"
								placeholder="가게명을 입력하세요." value="${list.getbName()}">
						</div>
						<!-- bName -->
						<div class="bTel">
							<input type="text" name="bTel" required="required"
								placeholder="00-0000-0000" value="${list.getbTel()}">
						</div>
						<!-- bTel -->
						<div class="bTime">
							<input type="text" name="bTime" required="required"
								placeholder="00:00~00:00" value="${list.getbTime()}">
						</div>
						<!-- bTime -->
						<div class="bImg">
							<input type="file" name="bImg" required="required">
							<!-- type ="file" 은 value 지정 x-->
							<!--  file은 actionName을 못 가져오니깐 ActionFactory 생성하지 X 서블릿에서 dao호출한다.-->
						</div>
						<!-- bImg -->
						...
						<div class="bLocation">
							<input type="text" id="sample4_roadAddress" name="bLocation"
								placeholder="도로명주소" required="required"
								value="${list.getbLocation()}"> <input type="button"
						...
						<div class="bInfo">
							<textarea name="bInfo" rows="9" cols="100"
								placeholder="소개글을 입력하세요." required="required">${list.getbInfo()}</textarea>
						</div>
						<!-- bInfo -->
						<div class="bHash">
